...
//  wave
//
//  Created by Burak  on 2020-12-02.
//
...
int data::data_reader(void){
    ...
    for( i=0; i<num_snippets; i++){ //for each 5s snippet
		...
		for(int j=0; j<num_segments2-values_per_snippet; j++){ //iterate through all segments of the other song until we reach the last n, where n is the length of a snippet
    		partial_match = 0; //Initialize for each comparison of a snippet to a set of values in the song signature
